...
##THIS FILE IS FOR RERUN OF DATA, SET LAST_TIMESTAMP AS THE PARTICPANTS START TIME AND END_TIMESTAMP AS THE PARTICPANTS END TIME, WILL PRINT TO PREDICTIONS.CSV
...
last_timestamp = datetime.fromisoformat("2025-09-23T03:04:28")

last_timestamp_str = last_timestamp.strftime("%Y-%m-%dT%I:%M:%S")

# for windowed testing
end_timestamp = datetime.fromisoformat("2025-09-23T03:21:37")
...
csv_writer.writerow(["timestamp", "valence", "arousal"])
...
while last_timestamp < end_timestamp:
    ...
    if new_ppg and is_valid_ppg(new_ppg):
        ...
        if data_hash in processed_hashes:
            ...
            if new_ppg and is_valid_ppg(new_ppg):
                # ... process segments, predictions ...
                last_valence = valence_preds[-1].item()
                last_arousal = arousal_preds[-1].item()
            else:
                last_valence = "null"
                last_arousal = "null"

            # write to CSV
            csv_writer.writerow([last_timestamp.isoformat(timespec='milliseconds') + "Z",
                                last_valence, last_arousal])
